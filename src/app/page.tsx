...
function Section({ id, children, style }: { id?: string; children: React.ReactNode; style?: React.CSSProperties }) {
  return (
    <section id={id} style={{ padding: "120px 48px", maxWidth: 1400, margin: "0 auto", ...style }}>
      {children}
    </section>
  )
}
...
function Counter({ to, suffix = "" }: { to: number; suffix?: string }) {
  ...
  useEffect(() => {
    ...
    const tick = (ts: number) => {
      ...
      setCount(p < 1 ? Math.floor(eased * to) : to)
...
function ProcessStep({ number, title, description }: { number: string; title: string; description: string }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "72px 1fr", gap: 32, paddingBottom: 48, borderBottom: `1px solid ${BORDER}` }}>
      <span style={{ fontSize: 44, fontWeight: 900, color: ACCENT, letterSpacing: "-0.04em", lineHeight: 1 }}>{number}</span>
      <div>
...
export default function Home() {
  ...
  return (
    <main style={{ background: BG, color: "#fff", minHeight: "100vh" }}>
      ...
      {/* HERO */}
      <section ref={heroRef} style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: "0 48px 80px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", bottom: -60, right: -80, fontSize: "clamp(200px, 30vw, 420px)", fontWeight: 900, color: "rgba(255,255,255,0.022)", userSelect: "none", pointerEvents: "none", letterSpacing: "-0.05em", lineHeight: 1 }}>01</div>

        <motion.div style={{ y: heroY, opacity: heroOpacity, maxWidth: 1400, width: "100%", margin: "0 auto" }}>
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }} style={{ display: "inline-flex", alignItems: "center", gap: 10, padding: "6px 18px", border: `1px solid ${BORDER}`, borderRadius: 100, marginBottom: 48, fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: MUTED }}>
            <motion.span animate={{ scale: [1, 1.4, 1] }} transition={{ duration: 2, repeat: Infinity }} style={{ width: 7, height: 7, borderRadius: "50%", background: ACCENT, display: "block" }} />
            Web Design &amp; Development Agency
          </motion.div>

          {["Built", "To Grow."].map((line, i) => (
            <div key={line} style={{ overflow: "hidden" }}>
              <motion.h1 initial={{ y: "105%" }} animate={{ y: 0 }} transition={{ duration: 1, ease: [0.76, 0, 0.24, 1], delay: 0.3 + i * 0.15 }} style={{ fontSize: "clamp(72px, 13vw, 200px)", fontWeight: 900, lineHeight: 0.88, letterSpacing: "-0.04em", margin: 0, color: i === 1 ? ACCENT : "#fff" }}>
                {line}
              </motion.h1>
            </div>
          ))}

          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.85 }} style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 40, marginTop: 64 }}>
            <p style={{ fontSize: "clamp(16px, 1.4vw, 20px)", color: MUTED, maxWidth: 480, lineHeight: 1.65, margin: 0 }}>
              High-performance websites that attract, convert, and retain — engineered for businesses that refuse to blend in.
            </p>
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              <motion.a href="#contact" whileHover={{ scale: 1.04, background: "#D4601F" }} whileTap={{ scale: 0.97 }} style={{ padding: "16px 40px", background: ACCENT, color: "#fff", borderRadius: 100, fontWeight: 700, fontSize: 15, letterSpacing: "-0.01em", display: "inline-block" }}>Get Started</motion.a>
              <motion.a href="#work" whileHover={{ borderColor: "rgba(255,255,255,0.3)" }} style={{ padding: "16px 40px", border: `1px solid ${BORDER}`, color: "#fff", borderRadius: 100, fontWeight: 600, fontSize: 15, letterSpacing: "-0.01em", display: "inline-block" }}>See Our Work</motion.a>
            </div>
          </motion.div>
        </motion.div>
      </section>
      ...
      <div style={{ padding: "28px 0", borderTop: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}` }}>
        <VelocityMarquee items={["Web Design", "Web Development", "Brand Identity", "GEO Optimised", "Conversion Focused", "Mobile First", "Sanity CMS", "Motion & Animation", "Results Driven", "Vercel Hosted"]} baseVelocity={-2} />
      </div>

      {/* STATS */}
      <Section>
        <ScrollReveal effect="fade-up"><Eyebrow label="Outcomes" /></ScrollReveal>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", borderRadius: 20, overflow: "hidden", border: `1px solid ${BORDER}` }}>
          {[
            ...
            <ScrollReveal key={label} effect="fade-up" delay={i * 0.1}>
              <div style={{ padding: "64px 48px", background: i === 1 ? `rgba(196,84,26,0.08)` : SURFACE, borderRight: i < 2 ? `1px solid ${BORDER}` : "none" }}>
                <div style={{ fontSize: "clamp(56px, 7vw, 96px)", fontWeight: 900, letterSpacing: "-0.04em", lineHeight: 1, color: i === 1 ? ACCENT : "#fff", marginBottom: 20 }}>
      ...
      <Section id="services" style={{ paddingTop: 0 }}>
        <ScrollReveal effect="fade-up">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 80, flexWrap: "wrap", gap: 24 }}>
            <div><Eyebrow label="Services" /><h2 style={{ fontSize: "clamp(40px, 5vw, 72px)", fontWeight: 900, letterSpacing: "-0.04em", lineHeight: 0.92 }}>What We<br /><span style={{ color: ACCENT }}>Build.</span></h2></div>
      ...
      <Section id="work" style={{ paddingTop: 0 }}>
        <ScrollReveal effect="fade-up">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 80, flexWrap: "wrap", gap: 24 }}>
            <div><Eyebrow label="Work" /><h2 style={{ fontSize: "clamp(40px, 5vw, 72px)", fontWeight: 900, letterSpacing: "-0.04em", lineHeight: 0.92 }}>Proof in<br /><span style={{ color: ACCENT }}>Results.</span></h2></div>
            <a href="/work" style={{ padding: "14px 32px", border: `1px solid ${BORDER}`, borderRadius: 100, fontSize: 14, fontWeight: 600, color: "#fff" }}>View all work →</a>
          </div>
        </ScrollReveal>
      ...
      <Section id="process" style={{ paddingTop: 0 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 100px", alignItems: "start" }}>
          <ScrollReveal effect="fade-up">
            <Eyebrow label="Process" />
            <h2 style={{ fontSize: "clamp(40px, 5vw, 72px)", fontWeight: 900, letterSpacing: "-0.04em", lineHeight: 0.92, position: "sticky", top: 120 }}>How We<br /><span style={{ color: ACCENT }}>Work.</span></h2>
          </ScrollReveal>
          <div style={{ display: "flex", flexDirection: "column", gap: 48 }}>
            {[
              ...
              <ScrollReveal key={step.number} effect="fade-up" delay={i * 0.08}><ProcessStep {...step} /></ScrollReveal>
            ))}
          </div>
        </div>
      </Section>
      ...
      {/* TESTIMONIAL */}
      <div style={{ padding: "120px 48px", borderTop: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}` }}>
        <ScrollReveal effect="scale">
          <div style={{ maxWidth: 880, margin: "0 auto", textAlign: "center" }}>
            <p style={{ fontSize: "clamp(20px, 2.8vw, 38px)", fontWeight: 700, lineHeight: 1.35, letterSpacing: "-0.02em", marginBottom: 48 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16 }}>
              <div style={{ width: 48, height: 48, borderRadius: "50%", background: `linear-gradient(135deg, ${ACCENT}, #6B1E00)` }} />
      ...
      <Section id="contact">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "start" }}>
          <ScrollReveal effect="fade-up">
            ...
            <h2 style={{ fontSize: "clamp(40px, 5vw, 72px)", fontWeight: 900, letterSpacing: "-0.04em", lineHeight: 0.92, marginBottom: 32 }}>Ready to<br /><span style={{ color: ACCENT }}>Grow?</span></h2>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              {["Response within 24 hours", "Free initial consultation", "No lock-in contracts"].map(text => (
                <div key={text} style={{ display: "flex", gap: 14, alignItems: "center", color: MUTED, fontSize: 15 }}>
                  <span style={{ color: ACCENT, fontSize: 10 }}>✦</span>{text}
                </div>
              ))}
            </div>
          </ScrollReveal>
          ...
          <ScrollReveal effect="fade-up" delay={0.15}>
            <div style={{ padding: 40, border: `1px solid ${BORDER}`, borderRadius: 20, background: SURFACE }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                {["Full Name", "Email Address", "Website URL (if you have one)"].map(ph => (
                  <input key={ph} placeholder={ph} style={{ width: "100%", padding: "16px 20px", background: "rgba(255,255,255,0.04)", border: `1px solid ${BORDER}`, borderRadius: 12, color: "#fff", fontSize: 15, outline: "none", fontFamily: "inherit" }} />
                ))}
                <textarea placeholder="Tell us about your project and goals" rows={4} style={{ width: "100%", padding: "16px 20px", background: "rgba(255,255,255,0.04)", border: `1px solid ${BORDER}`, borderRadius: 12, color: "#fff", fontSize: 15, outline: "none", resize: "none", fontFamily: "inherit" }} />
                <p style={{ color: MUTED, fontSize: 12 }}>Replace this with your GHL form embed. All data flows directly into your GHL CRM.</p>
                <motion.button whileHover={{ background: "#D4601F", scale: 1.02 }} whileTap={{ scale: 0.98 }} style={{ padding: 18, background: ACCENT, color: "#fff", border: "none", borderRadius: 12, fontWeight: 700, fontSize: 16, cursor: "pointer", fontFamily: "inherit", letterSpacing: "-0.01em" }}>
                  Send My Brief
                </motion.button>
              </div>
            </div>
      ...
      {/* FOOTER */}
      <footer style={{ borderTop: `1px solid ${BORDER}`, padding: "60px 48px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 32, maxWidth: 1400, margin: "0 auto" }}>
        <div>
